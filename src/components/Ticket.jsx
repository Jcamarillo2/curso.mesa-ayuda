...
//   )
    ...
    //   {estatus:'Cerrado', solicitante:'*'},
    //   {estatus:'Incluir nota', solicitante:'*'},  
    //   {estatus:'Reasignar', solicitante:'*'},
    // ];
    var mAcciones=[]
    // Si es solicitante
    if (user.id === post.idUsuario && post.status !== "En Cierre" && post.status !== "Cerrado")  { mAcciones.push({estatus:'Incluir nota'})    }
    if (user.id === post.idUsuario && post.status === "En Cierre") { mAcciones.push({estatus:'En Revisión'}, {estatus:'Cerrado'})}     
    if (user.id === post.idUsuario && post.status === "Cerrado") { mAcciones.push({estatus:'Reincidencia'})}     
...
//console.log (unique)
 const [ticketDate, setTicketDate] = useState( unique )
                ...
                <Menu
                    ...
                    >
                      ...
                      ticketDate.map((option, index) => (
                        <MenuItem key={option}  onClick={(event)=>onMenuItemClick(event, index, option)} selected={index===selectItem}>
                          {option}
                        </MenuItem>
                      ))
